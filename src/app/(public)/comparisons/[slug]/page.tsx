...
/**
 * Prerender every published comparisons article (playbook fix #5). Slugs that
...
export async function generateStaticParams() {
  const slugs = await publishedSlugs("comparisons");
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return buildBlogArticleMetadata("comparisons", slug);
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return renderBlogArticle("comparisons", slug);
}
